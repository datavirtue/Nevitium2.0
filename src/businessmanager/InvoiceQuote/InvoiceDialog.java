...
import businessmanager.Reports.ReportDialog;

import businessmanager.Common.JTextFieldFilter;
import businessmanager.Connections.MyConnectionsApp;
import businessmanager.Connections.ConnectionsShippingDialog;
import businessmanager.Common.NewEmail;
import businessmanager.Common.VATCalculator;
import businessmanager.Inventory.MyInventoryApp;
import businessmanager.InvoiceQuote.LayoutManager.DocumentElementTable;
import businessmanager.InvoiceQuote.LayoutManager.DocumentLayout;
import businessmanager.InvoiceQuote.LayoutManager.InvoicePrintPanel;
import businessmanager.InvoiceQuote.LayoutManager.PrintPreview;
...
public class InvoiceDialog extends javax.swing.JDialog  {
    ...
    /** This costructor is used to start a new invoice or to
      load a quote; Specify a zero quoteKey to create a new invoice*/
    public InvoiceDialog(java.awt.Frame parent, boolean modal, int quoteKey, GlobalApplicationDaemon application) {
         //TODO: stab int quoteKey here to load a quote
        super(parent, modal);
	...
	public void windowClosing(java.awt.event.WindowEvent e){
        ...
        invoice = new Invoice(application);  //blank invoice object
        invoice.setVAT(VAT);
        ...
        quote = true;//invoice is a quote with an invoice number unless saved
        savedQuote = false;
        setInvoiceNumber();
        ...
        if (c.equalsIgnoreCase("UPC")) upcCombo.setSelectedItem("UPC");
        if (c.equalsIgnoreCase("CODE")) upcCombo.setSelectedItem("Code");
        if (c.equalsIgnoreCase("DESC")) upcCombo.setSelectedItem("Desc");
    ...
    public InvoiceDialog(java.awt.Frame parent, boolean modal, GlobalApplicationDaemon application, int invoice_key) {
	...
	public void windowClosing(java.awt.event.WindowEvent e){
        ...
        invoiceKey = invoice_key;  //set the global private invoiceKey so the print() knows what to do
        ...
        }else viewReturnsButton.setVisible(false);
   ...
   private void setDefaultMessage(){
    ...
    private void getInvoice (int key) {
        ...
        invoice = new Invoice(application, key);
        invoice.setVAT(VAT);
        /* Removed 06-02-2011 */
        //invoice.setTaxes(taxRate1, taxRate2);
        ...
        if (!sta.equals("")) shipToTextArea.setText(sta);
    ...
    private void getQuote (int key) {
        ...
        tm.setRowCount(items.getRowCount());
        if (debug) System.out.println("qitems row count in id getQuote() :"+items.getRowCount());
    ...
    private void testPrintLayout(){
        ...
        if (debug) System.out.println("max per page: "+max_per_page);
        
        int number_of_invoice_items = invoice.getItemCount(); //actual: num_items - 1
        
        if (debug) System.out.println("number of invoice items: "+number_of_invoice_items);
        
        /* This only counts the base with no remainder */
        int number_of_pages = number_of_invoice_items / max_per_page;
        
        if (debug) System.out.println("number of calculated pages: "+number_of_pages);
        ...
        if (number_of_invoice_items <= max_per_page) {
            ...
            if (number_of_invoice_items >= max_per_page) end = max_per_page;
        }
    ...
    private void testprintPDF () {
        ...
        if (!rd.getState()){
            ...
            if (viewPrint || savedQuote) return;
        ...
        if (!DV.isFileAccessible(file, "PDF file")) return ;
        ...
        String statement = " BAL DUE";
            ...
            if (invoice.isPaid()) statement = " P A I D";
        ...
        try {
            ...
            String s = "BILL TO:";
            if (quote) s = "FOR:";
            if (!props.getProp("BILLTO").equals("")) s = props.getProp("BILLTO");
            ...
            }else cell = new PdfPCell (new Phrase(""));
 ...
 /*  END ITEM HEADER TABLE */
        ...
        foot.setWidths(ftheaderwidths);
         ...
         if (tax1.equalsIgnoreCase("GST")) tax1 = "GST Content";
         if (tax1.equalsIgnoreCase("VAT")) tax1 = "VAT Content";
...
/*  END COSMETICS */
            ...
            if (rd.isEmail()){
                ...
                String ea = invoice.getEmail();
                if (quote) ea = theQuote.getEmail();
   ...
   private boolean printReciept () {
       ...
       POSPrinter posPrinter = new POSPrinter(new java.awt.Font("Courier", java.awt.Font.BOLD, 8), true, width);
...
/*
    ...
    private float computeVAT( String inventory_desc, float model_unit_price, boolean model_GST){
        ...
        if (!VAT) return model_unit_price;
    ...
    private void computePrices () {
        ...
        if (debug) System.out.println("InvoiceDialog: totalTax1"+totalTax1+" TotalTax2"+totalTax2);
        // update tax fields
    ...
    private void miscAction () {
        ...
        if (mi.getStat()) addItem(mi.getItem(), false, -1, -1); //no replace and no insert
    ...
    private void addItem (Object [] rec, boolean addCat, int replaceRow, int insert){
        ...
        if (!code.trim().equals("MISC") && !code.trim().equals("DISC")){
            ...
            if (!isItemValid(rec, quantity, true)) return;
        }        
    ...
    private void initComponents() {
        ...
        invTable.setShowHorizontalLines(false);
    ...
    private ArrayList calcWeight(boolean show_total) {
        ...
        if (a.length < 1){
            ...
            if (show_total) javax.swing.JOptionPane.showMessageDialog(null, "Please select some items to calculate. (Hold Ctrl and Click each row.)");
            return null;
        }
        ...
        for (int i = 0; i < a.length; i++){
            ...
            if (al != null){
                ...
                try {
                    ...
                    if (debug) System.out.println("Table row "+ a[i]);
        ...
        if (show_total) javax.swing.JOptionPane.showMessageDialog(null, "Total Weight: "+ total_weight+ ' ' +measure);
            ...
            try{
             in = new BufferedReader(
                 ...
                 skip = false;
                  line = in.readLine();
                                try {
                                    if (line != null) k = Integer.valueOf(line);
                                    else skip = true;
                                } catch (NumberFormatException ex) {
                                    /* ignore bad numbers */
                                    skip = true;
...
//do big fat search: find the product based on the code recieved from upcField
                 ...
                 if (al != null ){
                    ...
                    for (int i = 0; i < al.size(); i++){
                     ...
                     String searchType = "";
                     if (col == 1) searchType = "UPC";
                     if (col == 2) searchType = "Code";
        ...
        } else JOptionPane.showMessageDialog(this, "Make sure Quantity is a valid number.","Form Problem!",  JOptionPane.OK_OPTION);
    ...
    private void post() {
        if (invTable.getRowCount() < 1) return;

        if (debug) System.out.println("Entering InvoiceDialog post() <>");
        if (!verifyInvoiceNumber(numberField.getText())) return;
        if (debug) System.out.println("InvoiceDialog post(), inv number ok <>");
        int r = tm.getRowCount();
        if (debug) System.out.println("InvoiceDialog post(), item row count: "+r);
        if (r < 1) return;
        ...
        ArrayList al;
        Object [] rec;
        if (debug) System.out.println("InvoiceDialog post(), reverifying inventory <>");
        for (int x = 0; x < r; x++){
            ...
            if (al != null){
                ...
                if (!isItemValid(rec, (Float)tm.getValueAt(x,2), false)) {
                    ...
                    if (debug) System.out.println("InvoiceDialog post(), item invalid <>");
        ...
        if (!postInvoice(paymentCheckBox.isSelected())){
            ...
            if (debug) System.out.println("Next Invoice Number: "+nextAutoInvoiceNumber);
            if (debug) System.out.println("Last Recorded Number: "+lastRecInvNum);
            if (new String(invoicePrefix+nextAutoInvoiceNumber).equals(lastRecInvNum)) incrInvoice();
    ...
    private void sendEmail(String toAddress, String file){
        ...
        if (!Tools.verifyEmailAddress(toAddress)) return;

        String type = props.getProp("INVOICE NAME");
        if (quote) type = props.getProp("QUOTE NAME");
    ...
    private void saveQuote() {
        ...
        computePrices();
            ...
            if (!good) return ;
            if (debug) System.out.println("Entering InvoiceDialog saveQuote(), inv num ok <>");
            ...
            if (quoteKey < 1){
                if (debug) System.out.println("<><> Quote post failed!! <><>");
                return;
            }
    ...
    private void setInvoiceNumber () {
        ...
        if (invoice.checkInvoiceNumber(invoicePrefix + props.getProp("NEXT NUMBER")) ) numberField.setText(invoicePrefix + props.getProp("NEXT NUMBER") );
        else {
    ...
    private void incrInvoice () {
     ...
     if (!DV.validIntString(props.getProp("NEXT NUMBER"))) {
        ...
        if (debug) System.out.println("Incrementing the invoice number.");
...
private boolean setQuoteNumber () {
    ...
    private void incrQuote () {
     ...
     if (!DV.validIntString(props.getProp("NEXT QUOTE"))) {
        ...
        if (debug) System.out.println("Incrementing the quote number.");
    ...
    private boolean postInvoice ( boolean payment) {
        if (debug) System.out.println("Entering InvoiceDialog postInvoice() <>");
        ...
        /* ?create a transaction system in case this fails at any point? */
            ...
            if (debug) System.out.println("Entering InvoiceDialog postInvoice(), inv num ok <>");
            ...
            if (invoiceKey < 1){
                if (debug) System.out.println("<><> Invoice post failed!! <><>");
                return false;
            }

            lastRecInvNum = invoice.getInvoiceNumber();
            if (debug) System.out.println("Last recorded invoice number: "+lastRecInvNum);
            ...
            if (payment) {
                ...
                pd.setVisible(true);
                invoice = new Invoice(application, invoiceKey);
                //paidCheckBox.setSelected(invoice.isPaid());
            }
    ...
    private void shipToAction() {
        ...
        if (invoice.getCustKey() > 0){
            ...
            if (a == 0){
                ...
                ConnectionsShippingDialog csd =
                        new ConnectionsShippingDialog(parentWin, true,
                        db, invoice.getCustKey(), true, application);

                int shipToKey = csd.getShipToKey();

                if (shipToKey > 0){

                    Object[] shipToAddr = db.getRecord("connship", shipToKey);

                    /*
                     Need to change the connship data to conn mapping

                     */
                    
                    Object[] pretend_conn = new Object[18];
                    pretend_conn[1] = shipToAddr[2];//company
                    pretend_conn[2] = shipToAddr[3];//formed name
                    pretend_conn[3] = "";//name
                    pretend_conn[4] = shipToAddr[4];//addr
                    pretend_conn[5] = shipToAddr[5];//addr2
                    pretend_conn[6] = shipToAddr[6];//city
                    pretend_conn[7] = shipToAddr[7];//region
                    pretend_conn[8] = shipToAddr[8];//zip
                    pretend_conn[17] = shipToAddr[9];//cntry code
                    
                    String [] ship = Tools.formatAddress(pretend_conn);
        ...
        int shipKey = cd.getReturnValue() ;  //real value

        if (shipKey == -1) return;//disable shipto again?

        shipToTextArea.setText("");


        if (shipKey != 0) {
        //Object [] custRecord = db.getRecord("conn",shipKey);

        //String zone = props.getProp("ADDRESS STYLE");

        String [] cust = formatAddress(shipKey, false);
        ...
        if (shipKey > 0) shipToTextArea.setEditable(false);
        invoice.setShipToAddress(shipToTextArea.getText());
        theQuote.setShipToAddress(shipToTextArea.getText());
   ...
   private void custAction () {
        ...
        if (k == -1) return; //canceld

        custTextArea.setText("");
        invoice.setCustKey(k); //sets the customer for this invoice
        cust_key = invoice.getCustKey(); //TODO: eliminate this behavior

        cd.dispose(); //dont call dispose before finsihing with method
        cd = null; //clean up quick

        if (cust_key != 0) {

            //String zone = props.getProp("ADDRESS STYLE");

            String [] cust = formatAddress(cust_key, true);
        ...
        if (cust_key > 0) custTextArea.setEditable(false);
        invoice.setCustomer(custTextArea.getText());

        upcField.requestFocus();
   }

   private String [] formatAddress(int key, boolean billTo) {

       String [] address;
       Object [] custRecord = db.getRecord("conn", key);

       address = Tools.formatAddress(custRecord);

        if (billTo){
            custTax1 = (Boolean) custRecord[18];
            custTax2 = (Boolean) custRecord[19];
        }
...
private void datePicker1PropertyChange(java.beans.PropertyChangeEvent evt) {//GEN-FIRST:event_datePicker1PropertyChange
    ...
    private void discountButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_discountButtonActionPerformed
        ...
        for (int r = 0; r < selectedRows.length; r++){
            ...
            if ((Boolean)invTable.getModel().getValueAt(selectedRows[r], 7)) tx2 = true;
            if ((Boolean)invTable.getModel().getValueAt(selectedRows[r], 6)) tx1 = true;
        }
        ...
        if (discountItem != null) this.addItem(discountItem, false, -1, insert);
