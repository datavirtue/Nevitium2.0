...
import businessmanager.InvoiceQuote.PDFInvoice;
import businessmanager.InvoiceQuote.Invoice;
import datavirtue.*;
import java.util.*;
...
//generates a report of outstanding invoices
        ...
        PDFReport cust = new PDFReport (file);
        ...
        do  {   //process each invoice
            ...
            Invoice invoice = new Invoice(application, (Integer)rm.getRealValue(0));
    ...
    public static boolean generateStatements (GlobalApplicationDaemon application, int key) {
        ...
        */
        ...
        //for (int i = 0; i < keys.size(); i++){
            ...
            Invoice invoice = new Invoice(application, (Integer) key);
...
/*  END COSMETICS */               
 ...
 public static boolean generateCustomerStatement (GlobalApplicationDaemon application, int custKey) {
        ...
        */
        ...
        if (custKey == 0) title = "Misc Invoice History";
        ...
        try {
            ...
            if (custKey > 0){
                Object cd [] = db.getRecord("conn", custKey);
                String [] addr = Tools.formatAddress(cd);
 ...
 /*  END ITEM HEADER TABLE */   
            ...
            ArrayList invoiceList = db.search("invoice", 11, Integer.toString(custKey) , false);
         ...
         for (int i=0; i < numInvoices; i++){  //invoice loop based on customerInvoices
                ...
                //itemsRow++;
                Invoice theInvoice = new Invoice(application, invoiceKey);
